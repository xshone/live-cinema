...
export class World {
    ...
    init(statsContainerId: string, videoElementId: string) {
        ...
        const groundCollider = new THREE.Mesh(
            new THREE.BoxGeometry(controlParams.hallWidth, controlParams.hallDepth, controlParams.wallThickness),
        )
        ...
        const transformedPositionArray = new Float32Array(screenGeometry.attributes.position.array.length)
        ...
        const generateScreen = () => {
            for (let rowIndex = 0; rowIndex <= this.screenHeightSegments; rowIndex++) {
                for (let colIndex = 0; colIndex <= this.screenWidthSegments; colIndex++) {
                    ...
                    transformedPositionArray[pointIndex * 3 + 2] = positionArray[pointIndex * 3 + 2] - curveDistance
                }
            }
            screenGeometry.setAttribute("position", new THREE.BufferAttribute(transformedPositionArray, 3))
        ...
        const generateHall = () => {
            ...
            for (let row = 0; row < chairRowCount; row++) {
                ...
                if (row === chairRowCount - 1) {
                    ...
                    platformBack.position.set(0, originPostionY + chairIntervalY + 0.2, positionZ + chairIntervalZ)
        ...
        toScreenLight.target.position.set(0, controlParams.screenLiftDistance, -controlParams.screenPositionZ)
        ...
        // ── Camera ────────────────────────────────────────────────────────────
        this.camera = new THREE.PerspectiveCamera(30, this.sceneSizes.width / this.sceneSizes.height, 1, 1000)
        ...
        try {
            ...
            console.warn("PositionalAudio: could not connect media element source (likely already connected):", e)
        }
        this.positionalAudio.setRefDistance(controlParams.refDistance)
        this.positionalAudio.setDirectionalCone(controlParams.coneInnerAngle, controlParams.coneOuterAngle, 0.1)
        ...
        directionalLightFolder.add(directionalLight, "intensity").name("Intensity").min(0).max(10).step(0.001)

        const screenLightFolder = this.gui.addFolder("Screen Light")
        screenLightFolder.add(fromScreenLight, "intensity").name("Glow Intensity").min(0).max(10).step(0.001)
        screenLightFolder.add(toScreenLight, "intensity").name("Projection Intensity").min(0).max(10).step(0.001)
